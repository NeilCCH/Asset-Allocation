...
const GAP_FORMULA: Record<string, string> = {
  退休金缺口:
    "退休時點所需資本 −（現有可投資資產成長 + 未來年結餘累積 + 勞退/月退現值）。所需資本 = 退休首年支出（今日生活支出 ×(1+通膨)^距退休年數）× 實質報酬年金現值因子〔實質報酬 =(1+年報酬)/(1+通膨)−1〕，同步反映退休期間支出逐年通膨、退休本金仍以年報酬成長；資產成長以年報酬複利；未來年結餘採現況收支（有填支出明細用明細、否則結餘級距）逐年投入，赤字則反向侵蝕，並於貸款繳清後結餘回升（兩階段）；勞退/月退以年報酬折現至退休時點抵減",
  保障缺口: "（未償負債 + 未來扶養支出 + 子女教育金） − （現有壽險保額 + 流動資產）。未來支出採未折現名目加總（身故當下所需保障，保守；三項同基礎，不折現）",
  ...
  return (
    <section className="hcr-card hcr-advisor">
      ...
      <p className="hcr-note">
        ...
        可累積資產 = 現有可投資資產以年報酬 {pctNum(model.params.returnRate)} 複利 + <strong>現況年結餘</strong>(有填支出明細以明細計、否則結餘級距)逐年投入以年報酬複利 + 勞退/月退以年報酬折現至退休時點之現值。
        <strong>年結餘為負（赤字）時，逐年侵蝕退休老本、以負值計入</strong>，不會被當成零；並於<strong>貸款繳清後結餘回升</strong>(兩階段),更貼近現實。
